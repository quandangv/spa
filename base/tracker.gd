...
const adjustment_speed = 500
const mouse_adjustment_range = 0.6
export var manual_adjustment_limit = Vector2(0.5, 2)
const shake_factor = 1
...
func _process(delta):
  ...
  global_position = manual_adjustment + shake.rotated(randf()*PI*2) * shake_factor
  if have_inputs:
...
func _input(event):
  ...
  if abs(factor) > 0.1:
    manual_factor = clamp(pow(1.1, factor) * manual_factor, manual_adjustment_limit.x, manual_adjustment_limit.y)
